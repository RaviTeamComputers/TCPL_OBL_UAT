pageextension 50449 pageextension50449 extends "Sales Order List"
{
    layout
    {
        ...
        addafter("Ship-to Country/Region Code")
        {
            ...
            {
                Caption = 'Outstanding Qty In Sqm';
                ApplicationArea = All;
            }
    ...
    actions
    {
        addafter(Dimensions)
        {
            ...
            action("Process All Sales Orders")
            {
                Caption = 'Process All Sales Orders';
                Promoted = true;
                RunObject = Codeunit "Process Sales Orders";
                ApplicationArea = All;
            }
            action("Process Sales Order")
            {
                ...
                trigger OnAction()
                var
                    ProcessOrder: Codeunit "Process Sales Orders";
                begin
                    CLEAR(ProcessOrder);
                    ProcessOrder.SetSalesOrder(rec."No.");
                    ProcessOrder.Run();
                end;
            }
            action(UpdatePicture)
            {
                ...
                begin
                    GeneralLedSetup.Get();
                    if UploadIntoStream('Update Picture', '', '', FileName, LInstream) then begin
                        GeneralLedSetup."Blob Storage".CreateOutStream(Oustrm);
                        CopyStream(Oustrm, LInstream);
                        GeneralLedSetup.Modify();
                        Message('Done');
                    end;
                end;
            }
