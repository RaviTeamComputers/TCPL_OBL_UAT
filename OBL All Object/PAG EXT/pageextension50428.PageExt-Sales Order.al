pageextension 50428 pageextension50428 extends 42
{
    ...
    layout
    {
        ...
        addafter("Sell-to City")
        {
            ...
            {
                ApplicationArea = All;
                Editable = "Discount Charges %Editable"; //TEAM 14763 11-06-23
        ...
        */
        ...
        addafter(Dimensions)
        {
            ...
            action("Calculate Structure")
            {
                ...
                begin
                    ...
                    Calculate_TradeCharge;
                    Calculate_DiscountCharge;
                    Calculate_Freight;
                    Calculate_Insurance;

                    Rec.Status := OrderStatus;
                    Rec."Calculate Structure" := true;

                    CurrPage.update(true);

                    Message('Structure Calculated Successfully..');
                end;
            }
        ...
        modify(Post)
        {
            ...
            var
                ...
                AmtGST: Decimal;
                SalesLine_Rec: Record "Sales Line"; //17699
                Custrec: Record Customer;
            begin
                //>>17699
                Custrec.get(rec."Sell-to Customer No.");
                IF (Custrec."State Code" <> '19') then begin
                    IF (Rec."Group Code" = '01') or (rec."Group Code" = '02') then begin
                        SalesLine_Rec.Reset();
                        SalesLine_Rec.SetRange("Document Type", SalesLine_Rec."Document Type"::Order);
                        SalesLine_Rec.SetRange("Document No.", Rec."No.");
                        SalesLine_Rec.SetRange(Type, SalesLine_Rec.Type::"Charge (Item)");
                        SalesLine_Rec.SetFilter("No.", 'INSURANCE');
                        if SalesLine_Rec.FindFirst() then begin
                            IF (SalesLine_Rec.Amount <= 0) then
                                Error('Insurance can not be posted in negative or equal to zero amount.');
                            IF (SalesLine_Rec."Unit Price" <= 0) then
                                Error('Insurance can not be posted in negative or equal to zero amount.');
                            IF (SalesLine_Rec."Amount Including VAT" <= 0) then
                                Error('Insurance can not be posted in negative or equal to zero amount.');
                        end else begin
                            Error('Please calculate Insurance for the order by using F7 or F11');
                        end;
                    end;
                end;
                //<<17699
            ...
            begin
                ...
                */
                ...
                Rec."Structure Freight Amount" := 0; //TEAM 14763 14-06-23

                CurrPage.Update;

                Check_CDValidation; //TEAM 14763 14-06-23
        ...
        addfirst("Request Approval")
        {
            ...
            action("Approve Credit")
            {
                ...
                begin
                    if rec.Status in [rec.Status::Approved, rec.Status::Released] then
                        Error('Release /Approved Status cannot be used');
                    SalesSetup.GET;
        ...
        addafter(CancelApprovalRequest)
        {
            ...
            action("OrderApprovals")
            {
                Caption = 'Order Approvals';
                Enabled = true;
                Promoted = true;
                PromotedIsBig = true;
                Visible = true;
...
*/
    ...
    */
    ...
    procedure Calculate_TradeCharge()
    var
    ...
    begin
        ...
        if Rec."CD Available for Utilisation" <> 0 then begin
            if Rec."CD Applicable" then begin
                        ...
                        if SalesHeader.FindFirst() then begin
                            ...
                            if not recSalesLine1.FindFirst then begin
                                ...
                                if rec."CD Available for Utilisation" < ((TotalLineAmount * 25) / 100) then begin
                                    Rec."Trade Discount" := rec."CD Available for Utilisation";
                                    //SalesHeader.Modify;
                                end else begin
                                    Rec."Trade Discount" := ((TotalLineAmount * 25) / 100);
                                    //SalesHeader.Modify;
                                end;
                            end else begin
                                if rec."CD Available for Utilisation" < ((TotalLineAmount * 25) / 100) then begin
    ...
    procedure Calculate_DiscountCharge()
    var
    ...
    begin
        ...
        if SalesHeader1.FindFirst then begin
            ...
            if chargeItemRec1.FindFirst then begin
                ...
                Rec."Discount Amount" := ABS((FinalAmount * SalesHeader1."Discount Charges %") / 100);
                //SalesHeader1.Modify;
            end;
    ...
    var
        ...
        GSTGroupCode: code[20];  //17699
        HSNSACCode: Code[10];  //17699
    begin
        CurrPage.update(true);
        ...
        if recSalesLine1.FindSet then begin
            GSTGroupCode := recSalesLine1."GST Group Code"; //17699
            HSNSACCode := recSalesLine1."HSN/SAC Code";  //17699
            repeat
                if (recSalesLine1."Trade Discount Amount" <> 0) or (recSalesLine1."Structure Discount Amount" <> 0) then begin
                    ...
                    if VarQtytoShip then
                        recSalesLine1.validate("Inv. Discount Amount", (((recSalesLine1."Trade Discount Amount" + recSalesLine1."Structure Discount Amount") / recSalesLine1."Qty. to Ship") * recSalesLine1.Quantity))
                    else
                        recSalesLine1.validate("Inv. Discount Amount", recSalesLine1."Trade Discount Amount" + recSalesLine1."Structure Discount Amount");
                    recSalesLine1.Modify;
                end else begin
                    //recSalesLine1.validate("Inv. Discount Amount", 0.01);
                    //recSalesLine1.Modify;
                end;
        ...
        if Rec."Calculate Insurance" then begin
            ...
            if RecSalesHeader.FindFirst then begin
                ...
                IF RecsalesLine.FindSet() then begin
                    repeat
                        if (RecsalesLine."Trade Discount Amount" <> 0) then
                            TotalDiscountAmount += ABS(RecsalesLine."Inv. Discount Amount" - RecsalesLine."Trade Discount Amount")
                        else
                            TotalDiscountAmount += RecsalesLine."Inv. Discount Amount";
                    until RecsalesLine.Next() = 0;
                end;
            end;
            //Discount Amount
                ...
                if chargeItemRec.FindFirst then
                    if chargeItemRec."Insurance Percentage" <> 0 then begin
                        ...
                        if not recSalesLine1.FindFirst then begin
                            ...
                            // recSalesLine1.validate("GST Group Code", chargeItemRec."GST Group Code");
                            // recSalesLine1.validate("HSN/SAC Code", chargeItemRec."HSN/SAC Code");
                            recSalesLine1.validate("GST Group Code", GSTGroupCode);
                            recSalesLine1.validate("HSN/SAC Code", HSNSACCode);
                            //<<17699
                            recSalesLine1.validate("Gen. Prod. Posting Group", chargeItemRec."Gen. Prod. Posting Group");
                            recSalesLine1.Insert;
                            recSalesLine1.UpdateAmounts();
                            ...
                            RecSalesHeader.setrange("No.", Rec."No.");
                            if RecSalesHeader.FindFirst then begin
                                Rec."Insurance Amount" := FinalInsuranceAmount;
                                //RecSalesHeader.Modify;
                            end;
    ...
    procedure Check_CDValidation()
    var
        RecSalesHeader: Record "Sales Header";
        RecSalesLine: Record "Sales Line";
        RecSalesRecsetup: Record "Sales & Receivables Setup";
        RecChargeItem: Record "Item Charge";
    begin
        Clear(RecSalesLine);
        RecSalesLine.Reset;
        RecSalesLine.SetRange("Document No.", Rec."No.");
        ...
        if RecSalesLine.FindSet then begin
            repeat
                RecSalesLine."Trade Discount Amount" := 0;
                RecSalesLine.validate("Inv. Discount Amount", 0);
                RecSalesLine."Structure Discount Amount" := 0;
                RecSalesLine.Modify;
            until RecSalesLine.Next = 0;
        end;
        ...
        RecChargeItem.Reset;
        RecChargeItem.setrange("No.", RecSalesRecsetup."Structure Frieght");
        if RecChargeItem.FindFirst then begin
            Clear(RecSalesLine);
            RecSalesLine.Reset;
            RecSalesLine.SetRange("Document No.", Rec."No.");
            RecSalesLine.SetRange(Type, RecSalesLine.Type::"G/L Account");
            RecSalesLine.SetRange("No.", RecChargeItem."GL Account");
            if RecSalesLine.FindSet then
        ...
        Clear(RecChargeItem);
        RecChargeItem.Reset;
        RecChargeItem.setrange("No.", RecSalesRecsetup."Insurance Structure");
        if RecChargeItem.FindFirst then begin
            Clear(RecSalesLine);
            RecSalesLine.Reset;
            RecSalesLine.SetRange("Document No.", Rec."No.");
            RecSalesLine.SetRange("No.", RecSalesRecsetup."Insurance Structure");
