...
ERROR(ErrorText,SL."No.",SL."Line No.",CheckFor);
    ...
    begin
        ...
        IF ("D4 Amount" <> 0) THEN
            if SeperatApprovalforWestZone() then
                MakeApprovalEntries(4, "D4 Amount")
            ELSE
                MakeApprovalEntries(3, "D4 Amount");
    ...
    procedure MakeApprovalEntries(ApprovalType: Option PCH,ZM,PSM,PAC,PACM; DiscountAmt: Decimal)
    var
        RecCustomer: Record Customer;
    ...
    begin
        CASE ApprovalType OF
            ...
            ApprovalType::PACM:
                BEGIN
                    RecSalesPerson.RESET;
                    RecSalesPerson.SETRANGE(Type, RecSalesPerson.Type::PACM);
                    IF RecSalesPerson.FINDFIRST THEN
                        REPEAT
                            MakeApprovalEntry("Salesperson Code", RecSalesPerson.Code, DiscountAmt);
                        UNTIL RecSalesPerson.NEXT = 0;
    ...
    local procedure SeperatApprovalforWestZone(): Boolean
    var
        SP: Record "Salesperson/Purchaser";
    begin
        Sp.RESET;
        SP.SETRANGE(type, SP.Type::PACM);
    ...
    procedure ApproveOrderStatus()
    var
        Selection: Integer;
        PApprove, CApprove, IApprove : Boolean;
    begin
        Selection := STRMENU(Text001, 1);
        IF Selection = 0 THEN
            EXIT;

        //'&Credit,&Price,&Inventory,&All
        CApprove := Selection in [1, 4];
        PApprove := Selection in [2, 4];
        IApprove := Selection in [3, 4];
        if PApprove then
            ApprovePrice(Rec);
        if CApprove then
            ApproveCredit(Rec);
    ...
    local procedure ApprovePrice(rec: Record "Sales Header")
    var
        ProcessSalesOrders: Codeunit "Process Sales Orders";
        AppEntry: Record "Approval Entry";
    begin
        IF NOT (UPPERCASE(USERID) IN ['ADMIN', 'MA028', 'FA007']) THEN
    ...
    local procedure ApproveCredit(rec: Record "Sales Header")
    var
        ProcessSalesOrders: Codeunit "Process Sales Orders";
        AppEntry: Record "Approval Entry";
    begin
        IF NOT (UPPERCASE(USERID) IN ['ADMIN', 'MA023', 'FA007', 'MA022', 'FA028', 'FA014', 'FA012', 'FA017', 'FA032', 'FA015', 'MA030']) then
            ERROR('You are not Authorized');
        ...
        if ReccSalesLine.FindSet() then
            repeat
                ...
                if ReccSalesLine.Type <> ReccSalesLine.Type::" " then begin
                    ...
                    TaxTransactionValue.SetCurrentKey("Tax Record ID", "Tax Type"); //MSKS
    ...
    begin
        ...
        SalesHeader.RESET;
        SalesHeader.SetCurrentKey("Sell-to Customer No."); //MSKS
    ...
    begin
        ...
        CustLedgerEntry.SetCurrentKey("Customer No.", "Posting Date", "Currency Code"); //MSKS
    ...
    var
        ...
        Text001: Label '&Credit,&Price,&Inventory,&All';
    ...
    begin
        ...
        ItemChargeAssgntSales.Reset();
        ItemChargeAssgntSales.SetCurrentKey("Document Type", "Document No.", "Document Line No.", "Line No.");//MSKS
        ItemChargeAssgntSales.SetRange("Document Type", SalesLine."Document Type");
        ItemChargeAssgntSales.SetRange("Document No.", SalesLine."Document No.");
        ...
        Initialize(SalesLine, ItemChargeAssgntLineAmt);
        UpdateQtyAssgnt;
        UpdateQty;
        ...
        AssignItemChargeSales.AssignItemCharges(SalesLine, AssignableQty, AssignableAmount, AssignableQty, AssignableAmount, 'By Amount');
    ...
    procedure CalcAmountToCustomer(CalcOutStandAmt: Boolean): Decimal
    var
        GSTSetup: Record "GST Setup";
    ...
    begin
        ...
        if SalesLine.FindFirst() then begin
            REPEAT
                if CalcOutStandAmt then
                    decAmounttoCust += (SalesLine.CalcAmountToCustomer() / SalesLine.Quantity) * SalesLine."Outstanding Quantity"
                else
                    decAmounttoCust += SalesLine.CalcAmountToCustomer();
            until SalesLine.Next() = 0;
