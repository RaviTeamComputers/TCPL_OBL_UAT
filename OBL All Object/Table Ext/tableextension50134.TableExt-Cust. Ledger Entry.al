tableextension 50134 tableextension50134 extends "Cust. Ledger Entry"
{
    ...
    fields
    {
        ...
        field(50055; Narration; Text[250])
        {
        ...
        field(50057; "Line Narration"; Text[250])
        {
            CalcFormula = Lookup("Posted Narration".Narration WHERE("Entry No." = FIELD("Entry No."),
                                                                     "Transaction No." = FIELD("Transaction No.")));
            Editable = false;
        ...
        field(50058; "Cash Discount"; Decimal)
        {
            //>>17699
            FieldClass = FlowField;
            CalcFormula = lookup("Sales Invoice Header"."Discount Amount" where("No." = field("Document No."), "Posting Date" = field("Posting Date"))); //17699
            // CalcFormula = Sum("Sales Invoice Line".Amount WHERE(Type = FILTER("Charge (Item)"), "Posting Date" = field("Posting Date"),
            //                                                                  "Document No." = FIELD("Document No."),
            //                                                                  "No." = FILTER('CASH DISCOUNT')));
            // //<<17699                                                                             
            Description = 'TRI V.D 16.09.10 New Field Added';

            // FieldClass = FlowField;
        }
        field(50059; "Ship To Name"; Text[100])
        {
            CalcFormula = Lookup("Sales Invoice Header"."Ship-to Name" WHERE("No." = FIELD("Document No.")));
            Editable = false;
            FieldClass = FlowField;
        }
        field(50060; "Ship To Address"; Text[100])
        {
            CalcFormula = Lookup("Sales Invoice Header"."Ship-to Address" WHERE("No." = FIELD("Document No.")));
            Editable = false;
            FieldClass = FlowField;
        }
        field(50061; "Ship To Address2"; Text[100])
        {
        ...
        field(50066; "Bill To Address"; Text[100])
        {
            CalcFormula = Lookup(Customer.Address WHERE("No." = FIELD("Customer No.")));
            Editable = false;
            FieldClass = FlowField;
        }
        field(50067; "Bill To Address 2"; Text[100])
        {
        ...
        field(50088; "Dealer Code N"; Code[20])
        {
            Description = 'TRI';
            FieldClass = FlowField;
            CalcFormula = lookup("Sales Invoice Header"."Dealer Code" where("No." = field("Document No."), "Posting Date" = field("Posting Date")));
            TableRelation = "Salesperson/Purchaser".Code WHERE("Customer No." = FILTER(<> ''));
        }
