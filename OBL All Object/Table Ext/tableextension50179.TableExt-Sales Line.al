tableextension 50179 tableextension50179 extends "Sales Line"
{
        ...
        modify(Quantity)// 15578
        {
            ...
            begin
                if Rec.Quantity = 0 then begin
                    ...
                    ItemChargeAssgntSales.SetCurrentKey("Applies-to Doc. Type", "Applies-to Doc. No.", "Applies-to Doc. Line No.");//MSKS
            ...
            begin
                ...
                if Quantity <> 0 then begin
                    IF BaseUOM."Qty. per Unit of Measure" <> 0 THEN begin
                        VALIDATE("Buyer's Price /Sq.Mt", ROUND((("Line Amount") / Quantity) / BaseUOM."Qty. per Unit of Measure", 0.01));
                    end;

                    if (Rec.Type = Rec.Type::Item) AND (Rec.Quantity <> 0) then //MSKS Quantity condition added 13062023 20:00
                        if Rec."Quantity in Sq. Mt." <> 0 then //TEAM 14763 15-06-23
                            Validate(d7);
                ...
                /* IF BaseUOM.GET("No.", "Unit of Measure Code") THEN;
                if Quantity <> 0 then begin
                    IF BaseUOM."Qty. per Unit of Measure" <> 0 THEN begin //TEAM 14763 15-06-23
                        VALIDATE("Buyer's Price /Sq.Mt", ROUND((("Line Amount") / Quantity) / BaseUOM."Qty. per Unit of Measure", 0.01));
        ...
        */
        ...
        field(60000; "TDS Nature of Deduction"; Code[10])
        {
            ...
            begin
                ...
                if CustomerAllowedSection.FindSet then
                    if Page.RunModal(Page::"Customer Allowed Sections", CustomerAllowedSection) = Action::LookupOK then
                        Rec.VALIDATE("TDS Nature of Deduction", CustomerAllowedSection."TDS Section");
            end;
    ...
    begin
        ...
        //"State Name" := SalesHeader."State Description";
        //END;
        //TRI 22.02.08 N.K Add Start
        refQtyinhand;
        //MS-PB BEGIN 161112
    ...
    */
    ...
    var
        ...
        EffectiveStartDate, EffectiveStartDate1 : Date;
        TDSPer, SurChargePer : Decimal;
    ...
    begin
        IF ("Document Type" = "Document Type"::"Credit Memo") THEN BEGIN
            ...
            */
            ...
            RecTaxRates.Reset;
            RecTaxRates.SetRange("Tax Type", 'TDS');
            if RecTaxRates.FindSet then begin
                repeat
                    Clear(ColumnCount);
                    Clear(GlobalTaxType);

                    if (StrPos(RecTaxRates."Tax Rate ID", 'IND') <> 0) then begin
                        if (StrPos(RecTaxRates."Tax Setup ID", 'CONT') <> 0) then begin
                            ColumnCount := 0;
                            GlobalTaxType := '';
                            TDSSetup.Get();
                            ...
                            FormatLine(RecTaxRates.ID);

                            if AttributeValue[1] = 'CONT' then begin
                                Evaluate(EffectiveStartDate1, AttributeValue[3]);

                                if EffectiveStartDate < EffectiveStartDate1 then begin
                                    EffectiveStartDate := EffectiveStartDate1;
                                    Evaluate(TDSPer, AttributeValue[8]);
                                    Evaluate(SurChargePer, AttributeValue[10]);
                                end;
    ...
    begin
        if (CurrFieldNo = 15) or (CurrFieldNo = 18) then begin

            "Trade Discount Amount" := 0;
            "Inv. Discount Amount" := 0;
            "Structure Discount Amount" := 0;
            if RecSalesHeader.GET("Document Type", "Document No.") THEN begin
                ...
                RecSalesHeader."Calculate Structure" := false;
                RecSalesHeader."Invoice Discount Amount" := 0;
                RecSalesHeader.Modify();
            end;
            ...
            //             RecSalesLine."Inv. Discount Amount" := 0;
            ...
            RecChargeItem.Reset;
            RecChargeItem.SETFILTER("No.", '%1|%2', RecSalesRecsetup."Structure Frieght", RecSalesRecsetup."Insurance Structure");
    ...
    var
        ...
        SalesLine: Record "Sales Line";
        IRate, IGSTAmt, SRate, SGSTAmt, CRate, CGSTAmt, TotalGSTAmt, TCSAmt, TCSRate, TDSAmt, TDSRate, InCessAmt, InCessRate : Decimal;
        GStTotal, TotalAmt, decAmounttoCust : Decimal;
    begin
        //TDS Calculation>>>>>>
        TDSAmt := 0;
        TDSRate := 0;
        ...
        if TaxTransactionValue.FindFirst() then
            repeat
                if TaxTransactionValue."Tax Type" = TDSSetup."Tax Type" then Begin
                ...
                if TaxTransactionValue."Tax Type" = TCSSetup."Tax Type" then Begin
                    case TaxTransactionValue."Value ID" of
                        1:
                            begin
                ...
                End;
                if TaxTransactionValue."Tax Type" = 'GST CESS' then Begin
                    case TaxTransactionValue."Value ID" of
                        10000:
                            begin
                                InCessAmt := TaxTransactionValue.Amount;
                                InCessRate := TaxTransactionValue.Percent;
                            end;
                    end;
                end;

                if TaxTransactionValue."Tax Type" = GSTSetup."GST Tax Type" then begin
                    case TaxTransactionValue."Value ID" of
                        6:
                            begin
                                SRate := TaxTransactionValue.Percent;
                                SGSTAmt := TaxTransactionValue.Amount;
                            end;
                        2:
                            begin
                                CRate := TaxTransactionValue.Percent;
                                CGSTAmt := TaxTransactionValue.Amount;
                            end;
                        3:
                            begin
                                IRate := TaxTransactionValue.Percent;
        ...
        decAmounttoCust += "Line Amount" - "Inv. Discount Amount" + IGSTAmt + CGSTAmt + SGSTAmt + TCSAmt + TDSAmt + InCessAmt;
        exit(decAmounttoCust);
    end;
