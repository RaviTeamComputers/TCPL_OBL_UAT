codeunit 50109 "Extend90"
{
    ...
    begin
        ...
        //>>17699
        PurchRcptHeader."Vendor Shipment No." := PurchaseHeader."Vendor Shipment No.";
        //<<17699
    end;
    ...
    local procedure OnAfterPurchRcptLineInsert(PurchaseLine: Record "Purchase Line"; PurchInvHeader: Record "Purch. Inv. Header"; PurchRcptHeader: Record "Purch. Rcpt. Header"; ItemLedgShptEntryNo: Integer; var PurchRcptLine: Record "Purch. Rcpt. Line"; var TempPurchLineGlobal: Record "Purchase Line" temporary; CommitIsSupressed: Boolean; var TempTrackingSpecification: Record "Tracking Specification" temporary; WhseReceive: Boolean; WhseShip: Boolean; xPurchLine: Record "Purchase Line"; TempWhseRcptHeader: Record "Warehouse Receipt Header");
    var
        AssociateVendorMgt: Codeunit 50054;
    begin
        if PurchRcptLine.IsTemporary then exit;
        PurchRcptLine."Capex No." := PurchaseLine."Capex No.";
        if (PurchRcptLine.Quantity <> 0) and (PurchRcptLine.Type = PurchRcptLine.Type::Item) then
            AssociateVendorMgt.GenerateItemLedgerEntries(PurchRcptLine); //Kulbhushan VMI
    end;
    ...
    // local procedure OnPostUpdateOrderLineOnBeforeInitQtyToInvoice(var TempPurchaseLine: Record "Purchase Line" temporary; WhseReceive: Boolean; WhseShip: Boolean);
    // begin
    //     if TempPurchaseLine."Document Type" <> TempPurchaseLine."Document Type"::"Return Order" then BEGIN
    //         TempPurchaseLine."Challan Quantity" := 0; //MSKS
    //         TempPurchaseLine."Actual Quantity" := 0;
    //         TempPurchaseLine."Accepted Quantity" := 0;
    //     END;
    // end;
    //
    ...
    begin
        ...
        IF RecPurchHeader."Return Order No" = '' THEN BEGIN
            ...
            // RecPurchHeader.Modify(true);  //17699
 ...
 */
    ...
    begin
        ...
        IF Usersetup.GET(USERID) THEN BEGIN
            ...
            Filterstring := ApprovalEntry.GETFILTERS;
            ...
            IF STRLEN(Filterstring) = 0 THEN BEGIN
                ...
                ApprovalEntry.SETRANGE(Status, Status::Open);
                ApprovalEntry.FILTERGROUP(0);
            END ELSE
    ...
    local procedure OnBeforeItemJnlPostLine(var ItemJournalLine: Record "Item Journal Line"; PurchaseLine: Record "Purchase Line");
    begin
        ItemJournalLine."Capex No." := PurchaseLine."Capex No.";
    end;
    //OSKS
    var
