codeunit 50110 "Extend12"
{
    ...
    local procedure GenerateSMSEntry(var GenJournalLine: Record "Gen. Journal Line")
    var
        ...
        IF cdPhone <> '' THEN BEGIN
            ...
            IF rGenJournalLine."Document Type" = rGenJournalLine."Document Type"::Payment THEN BEGIN
                txtMessage := 'Amount Received : ' + FORMAT(ABS(rGenJournalLine.Amount)) + ' from : ' + recCustomer.Name
                                  + ', Dated : ' + FORMAT(rGenJournalLine."Posting Date") + ' and Cheque No. : ' + rGenJournalLine."Cheque No." + ' - Team OBL';
    ...
    begin
        ...
        IF CustLedgEntry.Narration = '' then
            CustLedgEntry.Narration := GenJournalLine.Narration;

        GenerateSMSEntry(GenJournalLine);
    end;
    /* 
    ...
    begin
        ...
        if GLEntry.Narration = '' then
            GLEntry.Narration := GLEntry.Description;
    ...
    // begin
    //     IF CustLedgerEntry.Narration = '' then
    ...
    local procedure OnAfterInitVendLedgEntry(GenJournalLine: Record "Gen. Journal Line"; var VendorLedgerEntry: Record "Vendor Ledger Entry"; var GLRegister: Record "G/L Register")
    begin
        VendorLedgerEntry.VALIDATE(Year, DATE2DMY(VendorLedgerEntry."Posting Date", 3));
        VendorLedgerEntry.VALIDATE(Month, DATE2DMY(VendorLedgerEntry."Posting Date", 2));
