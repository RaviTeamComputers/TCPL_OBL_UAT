codeunit 50111 "BasecodeunitExt"
{
    ...
    begin
        ...
        if BankAccLedgEntry."Bal. Account Type" = BankAccLedgEntry."Bal. Account Type"::Customer then begin
            if RecCustomer.GET(BankAccLedgEntry."Bal. Account No.") then
            ...
            if BankAccLedgEntry."Bal. Account Type" = BankAccLedgEntry."Bal. Account Type"::Vendor then begin
                if RecVendor.GET(BankAccLedgEntry."Bal. Account No.") then
                ...
                if BankAccLedgEntry."Bal. Account Type" = BankAccLedgEntry."Bal. Account Type"::"G/L Account" then begin
                    if RecGLAccount.GET(BankAccLedgEntry."Bal. Account No.") then
                        BankAccReconLine.Name := RecGLAccount.Name;
                end else
                    if BankAccLedgEntry."Bal. Account Type" = BankAccLedgEntry."Bal. Account Type"::"Bank Account" then begin
                        if RecBank.GET(BankAccLedgEntry."Bal. Account No.") then
                            BankAccReconLine.Name := RecBank.Name;
                    end;
    end;
 ...
 */
    ...
    begin
        ...
        SalesOrderHeader."Group Code" := BlanketOrderSalesHeader."Group Code";  //17699
    ...
    begin
        if (SalesHeader.Status = SalesHeader.Status::Approved) or (SalesLine."Charge Item") then
            IsHandled := true;
        SalesHeader.CalcFields("State Description");
        SalesLine."State Name" := SalesHeader."State Description";
    ...
    [EventSubscriber(ObjectType::Codeunit, Codeunit::"Reservation Management", 'OnBeforeAutoReserveTransLine', '', false, false)]
    ...
    begin
        //MSKS
        BankAccountLedgerEntry."Issuing Bank" := GenJournalLine."Issuing Bank";
        BankAccountLedgerEntry."Description 2" := GenJournalLine.Description2;
        ...
        BankAccountLedgerEntry.Narration_OT := GenJournalLine.Narration;
    end;
    ...
    var
        BSO_Rec: Record "Sales Header";
        SO_Rec: Record "Sales Header";
    begin
        IF BSO_Rec.GET(BlanketOrderSalesLine."Document Type", BlanketOrderSalesLine."Document No.") then begin
            SO_Rec.Reset();
            ;
            SO_Rec.SetRange("Document Type", SalesOrderLine."Document Type");
            SO_Rec.SetRange("No.", SalesOrderLine."Document No.");
            if SO_Rec.FindFirst() then begin
                SO_Rec.Validate("Location Code", BSO_Rec."Location Code");
                SO_Rec."Posting No. Series" := BSO_Rec."Posting No. Series";
                SO_Rec.Modify;
            end;
        end;
    end;
    ...
    var
        BSOL_Rec: Record "Sales Line";
        SOL_Rec: Record "Sales Line";
    begin
        SOL_Rec.Reset();
        SOL_Rec.SetRange("Document Type", BSOL_Rec."Document Type");
        SOL_Rec.SetRange("Document No.", BSOL_Rec."Document No.");
        IF SOL_Rec.FindSet(true, false) then begin
            SOL_Rec.Validate("Location Code", BSOL_Rec."Location Code");
            SOL_Rec.Modify(true);
        end;
    end;
    //<<17699
    ...
    begin
        ...
        if (TransferLine."Item No." <> '') and (not ItemDescriptionIsNo) and (TransferLine.Description <> '') then begin
            ...
            then
                TransferLine.Validate(TransferLine."Item No.", Item."No.");
        ...
        if Item.TryGetItemNoOpenCard(ReturnValue, TransferLine.Description, false, true, true) then
            case ReturnValue of
                '':
                    TransferLine.Description := xTransferLine.Description;
                TransferLine."Item No.":
                    TransferLine.Description := xTransferLine.Description;
                else begin
                    CurrFieldNo := TransferLine.FieldNo(TransferLine."Item No.");
                    TransferLine.Validate("Item No.", CopyStr(ReturnValue, 1, MaxStrLen(Item."No.")));
                end;
            end;
    ...
    local procedure InsertBankAccReconciliationLine(var BankAccReconLine: Record "Bank Acc. Reconciliation Line"; var BankAccLedgEntry: Record "Bank Account Ledger Entry")
    begin
        BankAccReconLine.Narration := BankAccLedgEntry.Narration_OT;
    end;
    ...
    begin
        TransShptLine."Quality Code" := TransLine."Quality Code";
        TransShptLine."Size Code" := TransLine."Size Code";

    end;
    //<<16630

    //>>
    [EventSubscriber(ObjectType::Page, Page::"Item Card", 'OnBeforeCreateItemFromTemplate', '', false, false)]
    ...
    local procedure OnAfterReleaseSalesDoc(PreviewMode: Boolean; var SalesHeader: Record "Sales Header"; var LinesWereModified: Boolean)
    var
        Sms: Codeunit "SMS Sender - Due Date";
    begin
        //TCPL-7632-260516
        CLEAR(SMS);
        IF (SalesHeader.Status = SalesHeader.Status::Released) AND (SalesHeader."Document Type" = SalesHeader."Document Type"::Order) THEN
            SMS.CreateMsgOnDARealease(SalesHeader);
