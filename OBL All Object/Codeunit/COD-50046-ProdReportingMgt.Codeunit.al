codeunit 50046 "Prod. Reporting Mgt."
{
                ...
                REPEAT
                    ...
                    IF ProdOrderLine.FINDFIRST THEN
                        REPEAT
                            ...
                            ProdOrderLine."Prod. Reporting No." := ProdReportingLine."Document No."; //MSKS
                            ProdOrderLine."Prod. Reporting Line No." := ProdReportingLine."Line No.";//MSKS
    ...
    begin
        ...
        IF ProdReportingLine.FINDFIRST THEN
            REPEAT
                ...
                if (ProdReportingLine."Economic Quantity" + ProdReportingLine."Commercial Quantity") > ProdReportingLine."Qty. in CRT." then
                    Error('Total Commercial Quantity cannot be greater than Total Carton Production for Line No. %1', ProdReportingLine."Line No.");
    ...
    begin
        ...
        IF ProdReportingLine.FINDFIRST THEN
            REPEAT
                ...
                IF ProdOrderLine.FINDFIRST THEN
                    REPEAT
                        CallConsumption(ProdReportingHeader, 'CONSUMP', 'DEFAULT', ProdOrderLine);
                    // IF PartiallyConsumedRPO(ProdOrderLine."Prod. Order No.") and (PartiallyConsumed = false) then
                    //     PartiallyConsumed := TRUE;
                    UNTIL ProdOrderLine.NEXT = 0;
            UNTIL ProdReportingLine.NEXT = 0;

        IF PartiallyConsumedRPO(ProdReportingHeader."No.") THEN
    ...
    local procedure PartiallyConsumedRPO(ProdRepNo: Code[20]): Boolean
    var
        ProdRepLine: Record "Prod. Reporting Line";
    begin
        ProdRepLine.REset();
        ProdRepLine.SetAutoCalcFields();
        ProdRepLine.SETFILTER("Document No.", '%1', ProdRepNo);
        ProdRepLine.SetFilter("Consumption Remaining", '%1', True);
        IF ProdRepLine.FindFirst() then
            // repeat
            //        ProdRepLine.CalcFields("Consumption Remaining");
            // if ProdRepLine."Consumption Remaining" then
            EXIT(true)
        ELSE
            //      EXIT(false)
            //else
            EXIT(false);
        //until ProdRepLine.Next() = 0;
    end;
