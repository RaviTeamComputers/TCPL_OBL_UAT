codeunit 50008 "QD Test, PDF Creation & Email"
{
  ...
  END;
    ...
    begin
        ...
        if txtEmailID = '' then
            txtEmailID := 'donotreply@orientbell.com';
        ...
        IF PurchaseHeader.Status = PurchaseHeader.Status::Released THEN BEGIN
            ...
            IF rApprovalEntry.FIND('-') THEN BEGIN
                //>>17699
                IF txtEmailID = '' then begin
                    rUserSetup.RESET;
                    IF rUserSetup.GET(rApprovalEntry."Sender ID") THEN
                        rUserSetup.TESTFIELD("E-Mail");
                    txtEmailID := rUserSetup."E-Mail";
                end;
                //<<17699
        ...
        BodyText += '<br><br>';
        BodyText += 'For Orient Bell Limited  <br>';
        EmailMsg.Create(EmailAddressList, 'Approval Mail ' + rPurchHeader."No.", BodyText, true, EmailCCList, EmailBccList);///15578000
        Glsetup.Get();
        Clear(Outsrm);
        Clear(Instrm);
        Clear(FileName);
        ...
        Clear(TempBlob);
        DocumentAttc.Reset();
        DocumentAttc.SetRange("Table ID", 38);
        DocumentAttc.SetRange("No.", PurchaseHeader."No.");
        DocumentAttc.SetRange("Document Type", PurchaseHeader."Document Type"::Order);
        if DocumentAttc.FindFirst() then begin
            TempBlob.CreateInStream(Instrm);
            TempBlob.CreateOutStream(Outsrm);
            DocumentAttc."Document Reference ID".ExportStream(Outsrm);
            Clear(GLsetup."38 Table");
            GLsetup."38 Table".CreateOutStream(Outsrm);
            CopyStream(Outsrm, Instrm);
            GLsetup.Modify();
            GLsetup.SetAutoCalcFields(GLsetup."38 Table");
            Clear(TInstrm);
            GLsetup."38 Table".CreateInStream(TInstrm);
            FileName := PurchaseHeader."No." + '.' + DocumentAttc."File Extension";
            AatchmentType := UpperCase(DocumentAttc."File Extension");
            EmailMsg.AddAttachment(FileName, AatchmentType, TInstrm);
        end;
        ...
        DocumentAttc.Reset();
        DocumentAttc.SetRange("Table ID", 38);
        DocumentAttc.SetRange("No.", rPurchHeader."No.");
        DocumentAttc.SetRange("Document Type", rPurchHeader."Document Type"::Order);
        if DocumentAttc.FindFirst() then begin
            DocumentAttc."Document Reference ID".ExportStream(Outsrm);
            CopyStream(Outsrm, Instrm);
        end;
        ...
        // 000 EmailMsg.AddAttachment(rPurchHeader."No." + '.pdf', 'PDF', Instrm);
    ...
    var
        ...
        Text50046: Label '>Approve Or Reject';
        Text50047: Label '</a>';
        UserSetup, SPUserSetup : Record 91;
        WorkflowUserGroupMember: Record 1541;
        "Field": Record 2000000041;
        i: Integer;
        AttachmentManagment: Record 50117;
        FilePath: Text;
        FileName: Text;
        InstreamVar: InStream;
        OutstreamVar: OutStream;
    ...
    begin
        IF SPUserSetup.GET(SPCode) THEN;
        ...
        IF SPUserSetup."E-Mail" <> '' THEN BEGIN
            EmailAddressList.Add(SPUserSetup."E-Mail");
            CASE VendorReplica."Vendor Class" OF
                VendorReplica."Vendor Class"::Employee:
                    BEGIN
            ...
            AttachmentManagment.SETFILTER(AttachmentManagment."Source Document No.", '%1', VendorReplica."No.");
            IF AttachmentManagment.FINDFIRST THEN BEGIN
                REPEAT
                    AttachmentManagment.CALCFIELDS(Content);
                    IF AttachmentManagment.Content.HASVALUE THEN BEGIN
                        TempBlobCU.FromRecord(AttachmentManagment, AttachmentManagment.FieldNo(Content));
                        TempBlobCU.CreateInStream(InstreamVar);
                        //AttachmentManagment.Attachment.CreateOutStream(OutstreamVar);
                        //AttachmentManagment.Attachment.CreateInStream(InstreamVar);

                        EMailMsg.AddAttachment(AttachmentManagment."File Name" + '.' + AttachmentManagment.Extension, 'application/pdf', Base64.ToBase64(InstreamVar));
