codeunit 50002 "Report Mail Utility"
{
    ...
    var
        ...
        GInstream: array[2] of InStream;
        GTempBlob: Codeunit "Temp Blob";
    ...
    var
        ...
        txtFile: Text[30];
        Linstrm: array[2] of InStream;
        LOutstrm: array[2] of OutStream;
        LTempbBlob: array[2] of Codeunit "Temp Blob";
        ...
        LInsteB: InStream;
    begin
        GLSetup.SetAutoCalcFields("Blob Storage");
        GLSetup.Get();
        MPDFDetail.RESET;
        MPDFDetail.SETRANGE("PDF Mailed", FALSE);
        //MPDFDetail.SetRange(MPDFDetail."Sales Invoice No.", 'SIDRA/2324/00430', 'SIDRA/2324/00431');// 15800
        IF MPDFDetail.FINDFIRST THEN BEGIN
            REPEAT
                ...
                RecSalesInv.SetAutoCalcFields("Report Storage");
                IF RecSalesInv.FINDFIRST THEN
                    IF mycust.GET(RecSalesInv."Sell-to Customer No.") THEN BEGIN
                        ...
                        IF InCorrectMail = FALSE THEN BEGIN
                            RecSalesInv.CALCFIELDS(/* RecSalesInv."Amount to Customer" */ RecSalesInv."Qty In carton");
                            ...
                            /*  if File.Exists('C:\MailPDF\' + MPDFDetail."Document No." + '.pdf') then begin
                             ...
                             END; */
                            IF (STRPOS(mycust."E-Mail", ' ') = 0) AND (mycust."E-Mail" <> '') THEN BEGIN
                                EmailAddressList.Add(mycust."E-Mail"); // 15800 mycust."E-Mail"

                                EmailMsg.Create(EmailAddressList, 'Sales Invoice' + ' - ' + MPDFDetail."Sales Invoice No." + ' - ' + MPDFDetail.CustName, BodyText, true, EmailCCList, EmailBccList);
                                //EmailMsg.AddAttachment(MPDFDetail."Document No." + '.pdf', 'application/pdf', 'C:\MailPDF\' + FileName);
                                J := 1;
                                Clear(Linstrm);
                                //for J := 1 to 2 do begin

                                /* LTempbBlob[J].CreateOutStream(LOutstrm[J]);
                                LTempbBlob[J].CreateInStream(Linstrm[J]);

                                 */
                                Clear(GInstream);
                                RecSalesInv."Report Storage".CreateInStream(GInstream[1]);
                                FileName1 := MPDFDetail."Document No." + '.pdf';
                                EmailMsg.AddAttachment(FileName1, 'PDF', GInstream[1]);
                                Clear(LInsteB);
                                GLSetup."Blob Storage".CreateInStream(LInsteB);
                                EmailMsg.AddAttachment('OrientBellNewLaunches.jpg', 'JPG', LInsteB);

                                // CopyStream(LOutstrm[J], Linstrm[J]);

                                /* end else begin
                                    LTempbBlob[J].CreateOutStream(LOutstrm[J]);
                                    LTempbBlob[J].CreateInStream(Linstrm[J]);
                                    GLSetup."Blob Storage".CreateInStream(Linstrm[J]);
                                    CopyStream(LOutstrm[J], Linstrm[J]);
                                    FileName1 := MPDFDetail."Document No." + '.jpg';
                                    EmailMsg.AddAttachment(FileName1, 'JPG', Linstrm[J]);
                                end; */
                                // end;
    ...
    begin
        ...
        SalesInvHeader.SETRANGE("Posting Date", WORKDATE - 5, WORKDATE);
        SalesInvHeader.SETRANGE("Invoiced Mailed", FALSE);
        SalesInvHeader.SetAutoCalcFields("Report Storage");
        //  SalesInvHeader.SetRange("No.", 'SIDRA/2324/00430', 'SIDRA/2324/00431');// 15800
        IF SalesInvHeader.FINDFIRST THEN BEGIN
            REPEAT
                DocNoNew := SalesInvHeader."No.";
                CLEAR(PDF);
                ...
                IF Customer.GET(SalesInvHeader."Sell-to Customer No.") THEN BEGIN
                    ...
                    IF SalesInvHeader1.FINDFIRST THEN BEGIN
                        ...
                        IF recCustType.FINDFIRST THEN BEGIN
                            ...
                            IF (recCust."State Code" = '19') AND (SalesInvHeader."LC Number" = '') THEN BEGIN
                                IF SalesInvHeader1."Order No." = '' THEN BEGIN
                                    ...
                                    //RecRef.get(SalesInvHeader1.RecordId);
                                    GTempBlob.CreateOutStream(GOutsrm);
                                    GTempBlob.CreateInStream(GInstream[1]);

                                    Report50319.SaveAs('', ReportFormat::Pdf, GOutsrm);
                                ...
                                END ELSE BEGIN
                                    ...
                                    END ELSE BEGIN
                                        ...
                                        Report50393.SaveAs('', ReportFormat::Pdf, GOutsrm);
                                    END;
                                END;
                            END ELSE BEGIN
                                IF SalesInvHeader1."Order No." = '' THEN BEGIN
                                    ...
                                    GTempBlob.CreateOutStream(GOutsrm);
                                    GTempBlob.CreateInStream(GInstream[1]);
                                    Report50156.SaveAs('', ReportFormat::Pdf, GOutsrm);
                                END ELSE BEGIN
                                    Clear(GInstream);
                                    Clear(GOutsrm);
                                    Clear(GTempBlob);
                        ...
                        SaleInv.RESET;
                        SaleInv.SETRANGE("No.", SalesInvHeader."No.");
                        SaleInv.SetAutoCalcFields("Report Storage");
                        IF SaleInv.FINDFIRST THEN BEGIN
                            SaleInv."Invoiced Mailed" := TRUE;
                            Clear(GInstream);
                            Clear(GOutsrm);
                            GTempBlob.CreateInStream(GInstream[1]);
                            SaleInv."Report Storage".CreateOutStream(GOutsrm);
                            CopyStream(GOutsrm, GInstream[1]);
                            SaleInv.MODIFY;
                        END;
                    END;
                END;
