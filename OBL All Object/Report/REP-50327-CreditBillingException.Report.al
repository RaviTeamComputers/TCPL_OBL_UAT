    ...
    dataset
    {
        dataitem(Customer; Customer)
        {
            ...
            dataitem("Sales Line"; "Sales Line")
            {
                ...
                begin
                    ...
                    */
                    ...
                    Clear(GSTPercVar);
                    GSTPercVar := (("Sales Line"."Outstanding Amount (LCY)" * AmttoCustomerSalesLine("Sales Line")) / 100);
                    IF GSTPercVar > 0 then begin
                        IF "Sales Line".Status <> "Sales Line".Status::Released THEN BEGIN
                            decOpenOrderVal := "Sales Line"."Outstanding Amount (LCY)" + GSTPercVar
                        END;
                        IF "Sales Line".Status = "Sales Line".Status::Released THEN BEGIN
                            decReleasedOrderVal := "Sales Line"."Outstanding Amount (LCY)" + GSTPercVar;
                        END;
                    End else begin
                        IF "Sales Line".Status <> "Sales Line".Status::Released THEN BEGIN
                            decOpenOrderVal := "Sales Line"."Outstanding Amount (LCY)";
                        END;
                        IF "Sales Line".Status = "Sales Line".Status::Released THEN BEGIN
                            decReleasedOrderVal := "Sales Line"."Outstanding Amount (LCY)";
                        END;
                    end;
                    //<<17699
            ...
            begin
                ...
                // AmtToCust April To Current Month
                CLEAR(MTDSaleAmt);
                CustLedgerEntry1.RESET;
                CustLedgerEntry1.SetRange("Document Type", CustLedgerEntry1."Document Type"::Invoice);//17699
                CustLedgerEntry1.SETRANGE(CustLedgerEntry1."Customer No.", Customer."No.");
                CustLedgerEntry1.SETFILTER("Posting Date", '%1..%2', MTDFromDate, EndDate);
                if CustLedgerEntry1.FindSet() then
                    repeat
                        CustLedgerEntry1.CALCFIELDS(Amount);  //17699
                        MTDSaleAmt += CustLedgerEntry1.Amount;
                    until CustLedgerEntry1.Next() = 0;

                // AmtToCust Last One Year
                CLEAR(FY20SaleAmt);
                CustLedgerEntry1.RESET;
                CustLedgerEntry1.SetRange("Document Type", CustLedgerEntry1."Document Type"::Invoice); //17699
                CustLedgerEntry1.SETRANGE(CustLedgerEntry1."Customer No.", Customer."No.");
                CustLedgerEntry1.SETFILTER("Posting Date", '%1..%2', FY20StartDate, EndDate);
                if CustLedgerEntry1.FindSet() then
                    repeat
                        CustLedgerEntry1.CALCFIELDS(Amount);
                        FY20SaleAmt += CustLedgerEntry1.Amount;
                    until CustLedgerEntry1.Next() = 0;
                ...
                IF SalesLine.FINDSET THEN
                    REPEAT
                        IF SalesLine.Status <> SalesLine.Status::Released THEN BEGIN
                            ...
                            OpenOrderValue += SalesLine."Outstanding Amount (LCY)" + ((SalesLine."Outstanding Amount (LCY)" * 1.1918) / 100); // Here permission by Amit Sir
                        END;
                        IF SalesLine.Status = SalesLine.Status::Released THEN BEGIN
                            //16225  ReleasedOrderValue += SalesLine."Outstanding Amount (LCY)" + ((SalesLine."Outstanding Amount (LCY)" * SalesLine."GST %") / 100);
                            ReleasedOrderValue += SalesLine."Outstanding Amount (LCY)" + ((SalesLine."Outstanding Amount (LCY)" * 1.1918) / 100);
                        END;
