report 50350 "Management Sales Report (MIS)"
{
    ...
    ApplicationArea = all;
    RDLCLayout = '.\ReportLayouts\ManagementSalesReportMIS 12.rdl';
    ...
    dataset
    {
        dataitem(TargetVsAchievement; "Matrix Master")
        {
            ...
            var
                SalesDataMGT: Query 50028;
                MTDSalesDataMGT: Query 50028;
                YTDSalesDataMGT: Query 50028;
                CSalesDataMGT: Query 50028;
                CMTDSalesDataMGT: Query 50028;
                CYTDSalesDataMGT: Query 50028;
            begin
                InitializeVariables;
                IF NOT FirstTime THEN BEGIN
                    ...
                    //16767 LastYearMTDSales := GetLastRevenue(0, '', LYMonthStartDate, LYMonthEndDate, LastYearMTDQty);
                    //16767  LYMonthSales := GetLastRevenue(0, '', LYMonthStartDate, CALCDATE('-1Y+CM', ASonDate), TempVariable);

                    //Last QRT
                    GetSalesValue('', LYQTRStartDate, LYQTREndDate, LYQTRQty, LYQTRSales, FALSE);
                    //16767  LYQTRSales := GetLastRevenue(0, '', LYQTRStartDate, LYQTREndDate, LYQTRQty);
        ...
        dataitem(CollectionZone; Integer)
        {
            ...
            trigger OnAfterGetRecord()
            var
                DIMDebtorsCollectionMGT: Query 50029;
            begin
    ...
    var
        ...
        CalculateSummaryInventory: Query 50024;
        LYInventory: Decimal;
        CYInventory: Decimal;
        QryMonthlySales: Query 50042;
    ...
    var
        QryMonthlyWiseSalesData: Query 50043;
        Amt: Decimal;
        TotQty: Decimal;
        Qty: Decimal;
        TotAmt: Decimal;
    ...
    var
        CustAgeingMGT: Query 50031;
        Docum: Enum "Document Type Enum";
    ...
    begin
        custledentry.reset;
        custledentry.SetAutoCalcFields(Amount);
        custledentry.SetRange(custledentry."Posting Date", Sdate, Edate);
        custledentry.SetFilter("Document Type", '%1|%2', custledentry."Document Type"::Payment, custledentry."Document Type"::Refund);
        if custledentry.FindSet() then
            repeat
                Amt -= custledentry.Amount;
            until custledentry.Next() = 0;


        /* CLEAR(CustAgeingMGT);
        CustAgeingMGT.SETRANGE(CustAgeingMGT.Posting_Date, Sdate, Edate);
        CustAgeingMGT.SETFILTER(CustAgeingMGT.Document_Type, '%1|%2', Docum::Payment, Docum::Refund);
        CustAgeingMGT.OPEN;
 ...
 */
    ...
    var
        ...
        DocuEnum: Enum "Document Type Enum";
    begin
        SalesAmt := 0;
        SalesQty := 0;
        ...
        SalesJournalData.SETRANGE(SalesJournalData.DocumentType, DocuEnum::Invoice);
        ...
        //IF FFType=FFType::PCH THEN
        SalesJournalData.SETFILTER(SalesJournalData.Tableau_Zone, '<>%1', 'CKA'); //MSKS
        SalesJournalData.SETRANGE(SalesJournalData.DocumentType, DocuEnum::"Credit Memo");
        SalesJournalData.SETFILTER(SalesJournalData.PostingDate, '%1..%2', FromDate, ToDate);
        CASE FFType OF
            FFType::SP:
                BEGIN
