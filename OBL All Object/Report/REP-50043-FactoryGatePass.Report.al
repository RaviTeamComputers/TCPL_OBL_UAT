report 50043 "Factory Gate Pass"
{
    ...
    dataset
    {
        dataitem("Sales Shipment Header"; "Sales Shipment Header")
        {
            ...
            column(TOtalTCSInclcesAmt; TOtalTCSInclcesAmt)  //17141
            { }
            dataitem("Sales Shipment Line"; "Sales Shipment Line")
            {
                ...
                var
                    SalesInvoiceHedaerL: Record "Sales Invoice Line";
                begin
                    ...
                    Clear(QtyInWeight);
                    SalesInvoiceHedaerL.Reset();
                    SalesInvoiceHedaerL.SetRange("Document No.", Invno);
                    SalesInvoiceHedaerL.SetRange(Type, SalesInvoiceHedaerL.Type::Item);
                    SalesInvoiceHedaerL.SetFilter(Quantity, '<>%1', 0);
                    if SalesInvoiceHedaerL.FindSet() then
                        repeat
                            QtyInWeight += SalesInvoiceHedaerL."Gross Weight";


                        until SalesInvoiceHedaerL.Next() = 0;
            ...
            begin
                ...
                /* SIL.RESET;
                SIL.SETFILTER(SIL."Document No.", SalesInvoiceHeader."No.");
                SIL.SetFilter(Quantity, '<>%1', 0);
                IF SIL.FindSet() THEN begin
                    repeat
                        TradeDisAmt += SIL."Trade Discount Amount";
                        CashDisAmt += SIL."Structure Discount Amount";
                    until SIL.Next() = 0;
                    Clear(TotalDiscount);
                ...
                TCSentry.Reset();
                TCSentry.SetRange("Document No.", Invno);
                ...
                IF SalesInvoiceHeader.FIND('-') THEN BEGIN
                    ...
                    //17141
                    SalesInvoiceHeader.CalcFields(SalesInvoiceHeader."Invoice Discount Amount");
                    varDisamt1 := SalesInvoiceHeader."Invoice Discount Amount";
                    //17141 added
        ...
        layout
        {
            area(content)
            {
                group(group)
                {
                    ...
                    field(No1; SalesInvoiceHeader."No.")
                    {
                        ApplicationArea = All;
                    }
                }
    ...
    begin
        ...
        if PstdSalesInv.Get(DocumentNo, DocLineNo) then //begin
            if PstdSalesInv.Type <> PstdSalesInv.Type::" " then begin
                ...
                TradeDisAmt := SIL."Trade Discount Amount";
                CashDisAmt := SIL."Structure Discount Amount";
                varInvDis := SIL."Inv. Discount Amount"; //Added
                TotalDiscount := Abs(TradeDisAmt + CashDisAmt);
        ...
        Clear(TotalAmt);
        // TotalAmt := (LineAmt + IGSTAmt + SGSTAmt + CGSTAmt - TotalDiscount) - TDSAmt; //Original
