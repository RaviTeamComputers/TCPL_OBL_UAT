report 50037 "Target Vs Achievement GVT"
{
            ...
            begin
                ...
                IF "Matrix Master"."Type 1" <> 'CKA' THEN BEGIN
                    ...
                    WHILE SalesInvoicedDetails.READ DO BEGIN
                        ...
                        //  FTD += GetGSTBaseAmtPostedLine(SalesInvoicedDetails.Document_No_, SalesInvoicedDetails.Line_No_);
                        FTD += SalesInvoicedDetails.Line_Amount;
                    ...
                    WHILE SalesInvoicedDetails.READ DO BEGIN
                        ...
                        // MTD += GetGSTBaseAmtPostedLine(SalesInvoicedDetails.Document_No_, SalesInvoicedDetails.Line_No_);
                        MTD += SalesInvoicedDetails.Line_Amount;
                    ...
                    WHILE SalesInvoicedDetails.READ DO BEGIN
                        IF (DT2DATE(SalesInvoicedDetails.Make_Order_Date) = AsonDate - 3) THEN BEGIN
                            InvBookedAmt[1] += SalesInvoicedDetails.Line_Amount;
                        ...
                        IF (DT2DATE(SalesInvoicedDetails.Make_Order_Date) = AsonDate - 2) THEN BEGIN
                            ...
                            InvBookedAmt[2] += SalesInvoicedDetails.Line_Amount;
                        END;
                        IF (DT2DATE(SalesInvoicedDetails.Make_Order_Date) = AsonDate - 1) THEN BEGIN
                            ...
                            InvBookedAmt[3] += SalesInvoicedDetails.Line_Amount;
                        END;
                ...
                END ELSE BEGIN
                    ...
                    WHILE SalesInvoicedDetails.READ DO BEGIN
                        ...
                        IF InvReleaseDate = AsonDate - 3 THEN BEGIN
                            InvOrderAmt[1] += SalesInvoicedDetails.Line_Amount;


                            InvOrderQty[1] += SalesInvoicedDetails.Sum_Quantity_Base;
                        END;
                        IF InvReleaseDate = AsonDate - 2 THEN BEGIN
                            InvOrderAmt[2] += SalesInvoicedDetails.Line_Amount;
                            InvOrderQty[2] += SalesInvoicedDetails.Sum_Quantity_Base;
                        END;
                        IF InvReleaseDate = AsonDate - 1 THEN BEGIN
                            InvOrderAmt[3] += SalesInvoicedDetails.Line_Amount;
                            InvOrderQty[3] += SalesInvoicedDetails.Sum_Quantity_Base;
                        END;
                    END;
                END;
    ...
    var
        ...
        SalesInvoiceHea: Record "Sales Invoice Header";
        SalesInvoiceLi: Record "Sales Invoice Line";
    begin
        /* CLEAR(SalesJournalData);
        ...
        WHILE SalesJournalData.READ DO BEGIN
            ...
            SalesAmt += GetAmttoCustomerPostedLine(SalesJournalData.Document_No_, SalesJournalData.Line_No);
        END;
 */ //16767
        /*  SalesReturnJournalData.SETFILTER(CustomerNo, '%1', CustNo);
         ...
         END; */ //16767
        SalesInvoiceHea.Reset();
        SalesInvoiceHea.SetRange("Posting Date", StartDate, EndDate);
        SalesInvoiceHea.SetRange("Sell-to Customer No.", CustNo);
        if SalesInvoiceHea.FindSet() then
            repeat
                SalesInvoiceLi.Reset();
                SalesInvoiceLi.SetRange("Document No.", SalesInvoiceHea."No.");
                SalesInvoiceLi.SetRange(Type, SalesInvoiceLi.Type::Item);
                SalesInvoiceLi.SetFilter(Quantity, '<>%1', 0);
                SalesInvoiceLi.SetFilter("Item Category Code", '%1|%2|%3|%4|%5', 'M001', 'T001', 'D001', 'H001', 'SAMPLE');
                if SalesInvoiceLi.FindSet() then
                    repeat
                        SalesAmt += GetAmttoCustomerPostedLine(SalesInvoiceLi."Document No.", SalesInvoiceLi."Line No.")
                    until SalesInvoiceLi.Next() = 0;
            until SalesInvoiceHea.Next() = 0;


        SalCrMemoHdr.Reset();
        SalCrMemoHdr.SetRange("Posting Date", StartDate, EndDate);
        SalCrMemoHdr.SetRange("Sell-to Customer No.", CustNo);
        if SalCrMemoHdr.FindSet() then
            repeat
                SalCrMemoLine.Reset();
                SalCrMemoLine.SetRange("Document No.", SalCrMemoHdr."No.");
                SalCrMemoLine.SetRange(Type, SalCrMemoLine.Type::Item);
                SalCrMemoLine.SetFilter(Quantity, '<>%1', 0);
                SalCrMemoLine.SetFilter("Item Category Code", '%1|%2|%3|%4|%5', 'M001', 'T001', 'D001', 'H001', 'SAMPLE');
                if SalCrMemoLine.FindSet() then
                    repeat
                        SalesAmt -= GetAmttoCustomerPostedLine(SalCrMemoLine."Document No.", SalCrMemoLine."Line No.");
                    until SalCrMemoLine.Next() = 0;

            until SalCrMemoHdr.Next() = 0
    ...
    begin
        ...
        if PstdSalesInv.Get(DocumentNo, DocLineNo) then begin
            ...
            TRADEDis := PstdSalesInv."Trade Discount Amount";
            CAshDis := PstdSalesInv."Structure Discount Amount";

            TotalDisAmt := Abs(TRADEDis + CAshDis);
        ...
        Clear(TotalAmt);
        TotalAmt := (LineAmt + IGSTAmt + SGSTAmt + CGSTAmt - TotalDisAmt); //- TDSAmt;
        EXIT(ABS(TotalAmt));
    end;
