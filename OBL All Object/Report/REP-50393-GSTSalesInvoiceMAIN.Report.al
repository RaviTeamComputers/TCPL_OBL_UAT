report 50393 "GST Sales Invoice MAIN"
{
    // --MSVRN 091117 modified--
    DefaultLayout = RDLC;
    UsageCategory = ReportsAndAnalysis;
    ApplicationArea = all;
    RDLCLayout = '.\ReportLayouts\GSTSalesInvoiceMAIN_1.rdl';
        ...
        dataitem("Sales Invoice Header"; "Sales Invoice Header")
        {
            ...
            dataitem(CopyLoop; Integer)
            {
                ...
                dataitem("Sales Invoice Line"; "Sales Invoice Line")
                {
                    ...
                    column(Disamt; "Sales Invoice Line"."Structure Discount Amount")// 16630 ABS(Discount)
                    {
                    }
...
DetailedGSTLedgerEntry.NEXT = 0;
                        ...
                        /*  TotalVATAMT := 0;
                         ...
                         if "Sales Invoice Line".FindSet() then
                             repeat
                                 TotalVATAMT += "Sales Invoice Line"."Line Amount";
                        ...
                        /*  State1.RESET;
                         ...
                         IF State1.FIND('-') THEN
                             Stname := State1.Description; 16225 */
...
RecState.NEXT = 0;
                ...
                Outstand := ROUND((Cust1.Balance), 1, '=');//16630
 ...
 */
                ...
                IF Cust."GST Registration No." = '' THEN BEGIN
                    ...
                    IF NOT "Sales Invoice Header"."QR Code".HASVALUE THEN BEGIN
                        ...
                        TempBlob2.CreateInStream(InstreamVar); //17699
                        //APIManagement.CreateQRCode(PaymentText, TempBlob2);
                        //MSKS    
                        QRGenerator.GenerateQRCodeImage(PaymentText, TempBlob2);
                        "Sales Invoice Header"."QR Code".CreateOutStream(OutstreamVar);  //17699
                        CopyStream(OutstreamVar, InstreamVar);
                        "Sales Invoice Header".MODIFY;
                        "Sales Invoice Header".CALCFIELDS("QR Code"); //17699
                    END ELSE BEGIN

                        // TempBlob.Blob := "Sales Invoice Header"."QR Code";
                    END;
                    //MSKS
                END;
                //QRCode
                ...
                IF Cust."GST Registration No." = '' THEN BEGIN
                  ...
                  amttocustomer := "Sales Invoice Header"."Amount to Customer";
                  CompanyInformation.GET;
                  DynamicsPAYID := CompanyInformation."UPI ID"+'.'+DELCHR("Sales Invoice Header"."No.",'=','/\-')+FORMAT("Sales Invoice Header"."Posting Date",0,'<Day,2><Month,2><Year4>')+CompanyInformation."UPI Bank Payment Name";
            ...
            trigger OnPreDataItem()
            var
                SIH: Record "Sales Invoice Header";  //17699
                DateVar: Date;  //17699
            begin
                //>>17699
                DateVar := 20230603D;
