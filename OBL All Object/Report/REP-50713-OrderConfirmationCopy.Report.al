report 50713 "Order Confirmation Copy1"
{
    DefaultLayout = RDLC;
    RDLCLayout = '.\ReportLayouts\OrderConfirmationCopy.rdl';
    ...
    dataset
    {
        dataitem("Sales Header"; 36)
        {
            ...
            column(TINNoCust; '') // 16630 replace "T.I.N. No."
            {
            }
            ...
            column(pmt; "Sales Header"."PMT Code")
            {
            }
            ...
            column(TradeDiscount_SalesHeader; ABS("Sales Header"."Trade Discount"))
            {
            }
            column(Ship_pin; "Sales Header"."Ship to Pin")
            {
            }
            column(FrightCharge; Abs(FrightCharge))
            {
                DecimalPlaces = 0 : 2;
            }
            column(InsuranceAmount; ABS(InsuAMT))
            {
            ...
            dataitem(CopyLoop; 2000000026)
            {
                ...
                dataitem(PageLoop; 2000000026)
                {
                    ...
                    column(TinNo; '') // 16630 replace "T.I.N. No."
                    {
                    }
                    ...
                    dataitem(RoundLoop; 2000000026)
                    {
                        ...
                        column(CRate; CGSTPERCENT)
                        {
                        }
                        column(CAmount; TotalCGSTAMT)
                        {
                        }
                        column(SRate; SGSTPERCENT)
                        {
                        }
                        column(SAmount; TotalSGSTAMT)
                        {
                        }
                        column(IRate; IGSTPERCENT)
                        {
                        }
                        column(IAmount; TotalIGSTAMT)
                        {
                        }
                        column(URate; URate)
                        {
                        ...
                        column(res_qty; "Sales Line"."Reserved Quantity")
                        {
                        }
                        column(BuyersRatePerSqMtr; BuyersRatePerSqMtr)
                        {
                        }
                        column(MRP; SalesLine."MRP Price")
                        {
                        }
                        column(LineExciseAmt; 0)// 16630 blank SalesLine."Excise Amount"
                        {
                        }
                        column(ExciseAmount; ExciseAmount)
                        {
                        }
                        column(InvDisc; ABS("Sales Header"."Discount Amount"))
                        {
                        }
                        column(AddInvDisSUMAddInsuDisc; ABS(AddInvDisc + AddInsuDisc))
                        {
                        }
                        column(Charges; 0)// 16630 Charges
                        {
                        }
                        ...
                        column(NNCSalesLineLineAmt; LineAmt)//RB
                        {
                        }
                        ...
                        column(SalesLineTaxAmount; 0) // 16630 blank SalesLine."Tax Amount"
                        {
                            AutoFormatExpression = "Sales Header"."Currency Code";
                            AutoFormatType = 1;
                        }
                        ...
                        column(VATAmount; 0) // 16630 blank SalesLine."Service Tax SHE Cess Amount"
                        {
                            AutoFormatExpression = "Sales Header"."Currency Code";
                            AutoFormatType = 1;
                        }
                        ...
                        column(TotalItemAmt; TotalItemAmt)
                        {
                            ...
                            begin
                                ...
                                IF ShowInternalInfo THEN BEGIN
                                    /*   NNC_SalesLineExciseAmt += SalesLine."Excise Amount";
                                       ...
                                       NNC_SalesLineKKCessAmt += SalesLine."KK Cess Amount";*/ // 16630
                                    NNC_SalesLineAmtToCustomer += TotalAmtCust; // 16630 SalesLine."Amount To Customer"
                        ...
                        begin
                            ...
                            TotalIGSTAMT := TotalIGSTAmtLineSales("Sales Line");
                            TotalCGSTAMT := TotalCGSTAmtLineSales("Sales Line");
                            TotalSGSTAMT := TotalSGSTAmtLineSales("Sales Line");

                            if Currency.GET(SalesLine."Currency Code") then;
                            if "Sales Line"."Unit of Measure Code" = 'PCS.' then
                                LineAmt := ROUND("Sales Line"."Quantity (Base)" * "Sales Line"."Unit Price", Currency."Amount Rounding Precision")
                            else
                                LineAmt := ROUND("Sales Line"."Quantity (Base)" * "Sales Line"."Unit Price Excl. VAT/Sq.Mt", Currency."Amount Rounding Precision");
                            ...
                            clear(TotalItemAmt);
                            ItemAmtLine.reset();
                            ItemAmtLine.setrange("Document No.", "Sales Line"."Document No.");
                            ItemAmtLine.setrange(Type, ItemAmtLine.Type::Item);
                            if ItemAmtLine.findset() then
                                repeat
                                    TotalItemAmt += ROUND(ItemAmtLine."Quantity (Base)" * ItemAmtLine."Unit Price Excl. VAT/Sq.Mt", Currency."Amount Rounding Precision");
                                until ItemAmtLine.next() = 0;
                        ...
                        TotalAmount := SalesLine."Line Amount" - VATAmountLine."Invoice Discount Amount" + SalesLine."Excise Amount" + SalesLine."Tax Amount"
                          ...
                          SalesLine."KK Cess Amount";*/ //16630
                            ...
                            IF (Wt > 0) AND (SalesLine."Quantity (Base)" > 0) THEN BEGIN
                                "Sales Line".CALCFIELDS("Reserved Qty. (Base)");
                                ResWt := (SalesLine."Gross Weight" / SalesLine."Quantity (Base)") * "Sales Line"."Reserved Qty. (Base)";  //RK040121
                            END;
                        ...
                        begin
                            ...
                            /* CurrReport.CREATETOTALS(SalesLine."Line Amount", SalesLine."Inv. Discount Amount", SalesLine."Excise Amount", SalesLine."Tax Amount",
                             ...
                             CurrReport.CREATETOTALS(SalesLine."KK Cess Amount");*/ // 16630
                ...
                begin
                    ...
                    // 16630    SalesPostPrepmt.BuildInvLineBuffer2("Sales Header", PrepmtSalesLine, 0, PrepmtInvBuf);
            ...
            begin
                ...
                SL.SetFilter("No.", '%1', 'INSURANCE');
                if SL.FindSet() then
                    InsuAMT := SL."Line Amount";

                SL.SetRange(Type, SL.Type::"G/L Account");
                SL.SetRange("No.", '%1', 'Freight Outward');
                if SL.FindSet() then
                    FrightCharge := SL."Line Amount";
                ...
                FormatAddr.SalesHeaderShipTo(ShipToAddr, ShipToAddr, "Sales Header");//RB
    ...
    requestpage
    {
        ...
        trigger OnOpenPage()
        begin
            ArchiveDocument := SalesSetup."Archive Orders"; // 16630 "Archive Quotes and Orders"
    ...
    var
        ...
        TotalItemAmt: Decimal;
        ItemAmtLine: Record "Sales Line";
    ...
    procedure TotalIGSTAmtLineSales(T37: Record 37): Decimal
    var
        igst, igstTotal : Decimal;
        GSTper1: Decimal;
        cgstTOTAL: Decimal;
        TotalAmt: Decimal;
        GSTSetup: Record "GST Setup";
    ...
    procedure TotalCGSTAmtLineSales(T37: Record 37): Decimal
    var
        igst: Decimal;
        igstTotal: Decimal;
        sgst: Decimal;
    ...
    procedure TotalSGSTAmtLineSales(T37: Record 37): Decimal
    var
        igst: Decimal;
        igstTotal: Decimal;
        sgst: Decimal;
