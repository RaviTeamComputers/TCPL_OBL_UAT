page 50040 "Vendor Requisition"
{
            ...
            group("Request Approval")
            {
                ...
                action(SendApprovalRequest)
                {
                    ...
                    begin
                        ...
                        if rec."Vendor Class" = rec."Vendor Class"::Importer then
                            rec.TestField("State Code", '99');
    ...
    begin
        ...
        IF (Rec."P.A.N. No." = '') OR (UPPERCASE(USERID) = 'FA006') THEN
            "P.A.N. No.Editable" := TRUE;

        IF (Rec.Name <> '') AND (UPPERCASE(USERID) = 'FA006') THEN
            NameEditable := TRUE
        ELSE
            IF Rec.Name = '' THEN
                NameEditable := TRUE;

        IF (Rec."Bank A/c" <> '') AND (UPPERCASE(USERID) = 'FA006') THEN
            "BankA/cEditable" := TRUE
        ELSE
