codeunit 51000 BaseCodeunit
{
    ...
    begin
        ...
        PurchRcptLine.SETFILTER("Posting Date", '>%1', 20230331D);
        PurchRcptLine.SETRANGE("Buy-from Vendor No.", PurchaseHeader."Buy-from Vendor No.");
        PurchRcptLine.SETFILTER("Location Code", MainLoc);
    END;

    // [EventSubscriber(ObjectType::Codeunit, Codeunit::"Purch.-Get Receipt", 'OnBeforeInsertInvoiceLineFromReceiptLine', '', true, true)]
    // local procedure OnBeforeInsertInvoiceLineFromReceiptLine(PurchRcptHeader: Record "Purch. Rcpt. Header"; PurchHeader: Record "Purchase Header")
    // begin
    //     if (PurchHeader."Vendor Invoice No." = '') And (PurchHeader."Vendor Invoice Date" = 0D) then begin
    ...
    //         PurchHeader."E-Way Bill No." := PurchRcptHeader."E-Way Bill No.";
    //     end;
    //     PurchHeader."Posting Date" := Today;
    //     PurchHeader.VALIDATE("GRN Date", PurchRcptHeader."Posting Date");
    //     PurchHeader."Due Date Calc. On" := PurchRcptHeader."Due Date Calc. On";
    //     PurchHeader.VALIDATE("Payment Terms Code", PurchRcptHeader."Payment Terms Code");
    //     PurchHeader.NOE := PurchRcptHeader.NOE;
    //     PurchHeader.modify();
    ...
    //                 PurchaseHeader."Vendor Invoice No." := RecPurchRecptHeader."Vendor Invoice No.";
    //                 PurchaseHeader."Vendor Invoice Date" := RecPurchRecptHeader."Vendor Invoice Date";
    //                 PurchaseHeader."E-Way Bill No." := RecPurchRecptHeader."E-Way Bill No.";
    ...
    local procedure OnBeforeInsertInvoiceLineFromReceiptLine(var PurchaseHeader: Record "Purchase Header"; var PurchRcptHeader: Record "Purch. Rcpt. Header"; var PurchRcptLine: Record "Purch. Rcpt. Line"; var TransferLine: Boolean)
    begin
        if (PurchaseHeader."Vendor Invoice No." = '') And (PurchaseHeader."Vendor Invoice Date" = 0D) then begin
            PurchaseHeader."Vendor Invoice No." := PurchRcptHeader."Vendor Invoice No.";
            PurchaseHeader."Vendor Invoice Date" := PurchRcptHeader."Vendor Invoice Date";
            PurchaseHeader."E-Way Bill No." := PurchRcptHeader."E-Way Bill No.";
        end;
        PurchaseHeader."Payment Terms Code" := PurchRcptHeader."Payment Terms Code";
        PurchaseHeader.Validate("Posting Date", WorkDate());
        PurchaseHeader.VALIDATE("GRN Date", PurchRcptHeader."Posting Date");
        PurchaseHeader."Due Date Calc. On" := PurchRcptHeader."Due Date Calc. On";
        PurchaseHeader.NOE := PurchRcptHeader.NOE;
        PurchaseHeader.ValidatePaymentTerms();
        //PurchaseHeader.modify();
    end;

    [EventSubscriber(ObjectType::Page, Page::"Purchase Invoice", 'OnAfterValidateEvent', 'Payment Terms Code', false, false)]// 5706
    local procedure OnAfterValidatePaymentTermsCode(var Rec: Record "Purchase Header"; var xRec: Record "Purchase Header");
    var
    begin
        rec.ValidatePaymentTerms();
    end;
    ...
    local procedure OnBeforeInsertInvLineFromRcptLine(PurchRcptLine: Record "Purch. Rcpt. Line"; var PurchLine: Record "Purchase Line");
    var
    begin
        if PurchRcptLine."HSN/SAC Code" <> '' then begin
            IF PurchLine."Document Type" = PurchLine."Document Type"::Invoice THEN begin
                PurchLine."GST Group Code" := PurchRcptLine."GST Group Code";
                PurchLine.Validate("HSN/SAC Code", PurchRcptLine."HSN/SAC Code");
                PurchLine."GST Credit" := PurchRcptLine."GST Credit";
                UpdateITC(PurchLine);
            end;
        end;
    end;
 ...
 */
    ...
    */
    ...
    begin
        ...
        /* IF not (UPPERCASE(USERID) IN ['ADMIN', 'OBLTILES\C', 'OBLTILES\D', 'OBLTILES\E', 'OBLTILES\F', 'OBLTILES\G', 'OBLTILES\N', 'OBLTILES\Team1']) THEN
    ...
    [EventSubscriber(ObjectType::Table, Database::"Assembly Header", 'OnBeforeValidateDates', '', false, false)]// 5706
    local procedure OnBeforeValidateDates(var AssemblyHeader: Record "Assembly Header"; var IsHandled: Boolean);
    var
    begin
        if AssemblyHeader."Transfer Order No." <> '' then
            IsHandled := true;
    end;
    ...
    */
    ...
    [EventSubscriber(ObjectType::Codeunit, Codeunit::"Production Journal Mgt", 'OnInsertConsumptionItemJnlLineOnBeforeValidateQuantity', '', false, false)]// 5706
    ...
    begin
        if NeededQty <> 0 then Begin
            if Item.Get(ItemJnlLine."Item No.") then
                if (Item."Rounding Precision" > 0) and (ItemJnlLine."Prod. Reporting No." = '') then
    ...
    var
        MachineCenterRec: record 99000758;
        vItem: Record item;

    begin
        ItemJournalLine.VALIDATE("Posting Date", ProdOrderLine."Due Date");
        IF ItemJournalLine.Type = ItemJournalLine.Type::"Machine Center" THEN BEGIN
            IF MachineCenterRec.GET(ItemJournalLine."No.") THEN
                IF MachineCenterRec."Output UOM" THEN
                    ItemJournalLine.VALIDATE("Unit of Measure Code", 'CRT');
        END ELSE
            ItemJournalLine.VALIDATE("Unit of Measure Code", ProdOrderLine."Unit of Measure Code");
        //TCPL-7632-250516
        //TCPL-7632-250516
        ItemJournalLine."Routing Link Code" := ProdOrderRtngLine."Routing Link Code";  //TRI V.D 21.06.10 ADD
        IF vItem.GET(ProdOrderLine."Item No.") THEN                                                     //TRI-VKG ADD
            ItemJournalLine.VALIDATE("Gen. Prod. Posting Group", vItem."Gen. Prod. Posting Group");//TRI-VKG ADD
                                                                                                   //TCPL-7632-250516
    ...
    local procedure PurHeaderOnAfterCopyBuyFromVendorFieldsFromVendor(var PurchaseHeader: Record "Purchase Header"; xPurchaseHeader: Record "Purchase Header"; Vendor: Record Vendor)
    var
    begin
        if xPurchaseHeader."Location Code" <> '' then begin
            PurchaseHeader."Location Code" := xPurchaseHeader."Location Code";
        end;
    end;

    [EventSubscriber(ObjectType::table, Database::"Purchase Line", 'OnAfterValidateEvent', 'No.', false, false)]// 5706
    local procedure PurchLineonAfterAddItem(var Rec: Record "Purchase Line"; var xRec: Record "Purchase Line"; CurrFieldNo: Integer)
    var
        HSN: Record "HSN/SAC";
    begin
        UpdateITC(rec);
    ...
    [EventSubscriber(ObjectType::table, Database::"Purchase Line", 'OnAfterValidateEvent', 'HSN/SAC Code', false, false)]// 5706
    local procedure PurchLineonAfterHSNSAcCode(var Rec: Record "Purchase Line"; var xRec: Record "Purchase Line"; CurrFieldNo: Integer)
    var
        HSN: Record "HSN/SAC";
    begin
        UpdateITC(Rec);
    ...
    local procedure OnAfterDescriptionPurchaseLineInsert(PurchRcptLine: Record "Purch. Rcpt. Line"; var NextLineNo: Integer; var PurchLine: Record "Purchase Line"; var TempPurchaseLine: Record "Purchase Line" temporary);
    var
    begin
        if PurchRcptLine."HSN/SAC Code" <> '' then begin
            IF PurchLine."Document Type" = PurchLine."Document Type"::Invoice THEN begin
    ...
    local procedure OnAfterAccountNoOnValidateGetVendorAccount(CallingFieldNo: Integer; var GenJournalLine: Record "Gen. Journal Line"; var Vendor: Record Vendor);
    var
    begin
    ...
    local procedure OnBeforeInitGenJnlLine(InvoicePostingBuffer: Record "Invoice Posting Buffer" temporary; PurchHeader: Record "Purchase Header"; var GenJnlLine: Record "Gen. Journal Line"; var IsHandled: Boolean);
    var
    Begin
        GenJnlLine."Location Code" := PurchHeader."Location Code";
        GenJnlLine.Narration := PurchHeader."Posting Description New";
    End;
    ...
    Begin
        ...
        GenJournalLine.Narration := PurchaseHeader."Posting Description New";
    End;
    ...
    var
    Begin
        GenJournalLine."Location Code" := CustLedgerEntry."Location Code";
    ...
    var
    Begin
        GenJournalLine."Location Code" := VendLedgerEntry."Location Code";
        GenJournalLine.Narration := VendLedgerEntry.Description;
    End;

    [EventSubscriber(ObjectType::Table, DAtabase::"Gen. Journal Line", 'OnAfterCopyGenJnlLineFromPaymentVendLedgEntry', '', false, false)]
    ...
    var
    Begin
        GenJournalLine."Location Code" := VendLedgEntry."Location Code";
        GenJournalLine.Narration := VendLedgEntry.Description;
    End;
    ...
    Begin
        ...
        GLEntry.Narration_OT := GenJournalLine.Narration;
    End;
    ...
    Begin
        ...
        VendorLedgerEntry.Narration_OT := GenJournalLine.Narration;
    End;

    // [EventSubscriber(ObjectType::table, Database::"Sales Line", 'OnBeforeTestStatusOpen', '', false, false)]
    // local procedure OnBeforeTestStatusOpenSalesLineQty(CallingFieldNo: Integer; var IsHandled: Boolean; var SalesHeader: Record "Sales Header"; var SalesLine: Record "Sales Line"; var StatusCheckSuspended: Boolean; xSalesLine: Record "Sales Line")
    // begin
    //     if SalesLine."Document Type" = SalesLine."Document Type"::Order then
    //         if CallingFieldNo = SalesLine.FieldNo(SalesLine.Quantity) then begin
    //             IF UPPERCASE(USERID) IN ['FA028', 'DE002', 'DE012', 'DE011', 'DE014', 'DE018', 'DE018', 'DE016'] THEN
    //                 IsHandled := true;
    //         end;
    // End;

    procedure UpdateITC(Var PurchLine: Record "Purchase Line")
    var
        hsn: record "HSN/SAC";
    begin
        IF PurchLine."Document Type" = PurchLine."Document Type"::Invoice THEN
            IF hsn.GET(PurchLine."GST Group Code", PurchLine."HSN/SAC Code") THEN BEGIN
                IF hsn.Type = hsn.Type::HSN THEN
                    PurchLine."ITC Type" := PurchLine."ITC Type"::"Input Goods"
                ELSE
                    IF hsn.Type = hsn.Type::SAC THEN
                        PurchLine."ITC Type" := PurchLine."ITC Type"::"Input Servce"
                    ELSE
                        IF PurchLine."HSN/SAC Code" = '' THEN
                            PurchLine."ITC Type" := PurchLine."ITC Type"::" ";
                // MODIFY
            END;
    ...
    begin
        Rec."State Code" := Rec."state code New";
    end;
    ...
    begin
        PurchInvLine."Source Order No." := PurchLine."Source Order No.";
    end;

    //
    [EventSubscriber(ObjectType::Codeunit, Codeunit::"Blanket Sales Order to Order", 'OnAfterInsertSalesOrderLine', '', false, false)]
    local procedure OnAfterInsertSalesOrderLine(var SalesOrderLine: Record "Sales Line"; SalesOrderHeader: Record "Sales Header"; BlanketOrderSalesHeader: Record "Sales Header"; BlanketOrderSalesLine: Record "Sales Line");
    var
    Begin
    ...
    [EventSubscriber(ObjectType::Codeunit, Codeunit::"Blanket Sales Order to Order", 'OnCreateSalesHeaderOnAfterSalesOrderHeaderInitFromSalesHeader', '', false, false)]
    local procedure OnCreateSalesHeaderOnAfterSalesOrderHeaderInitFromSalesHeader(var SalesHeader: Record "Sales Header"; var SalesOrderHeader: Record "Sales Header");
    var
    Begin
        SalesOrderHeader."Group Code" := SalesHeader."Group Code";
    End;

    //OSKS
    [EventSubscriber(ObjectType::Codeunit, Codeunit::"TransferOrder-Post Shipment", 'OnAfterCreateItemJnlLine', '', false, false)]
    local procedure OnAfterCreateItemJnlLine(var ItemJournalLine: Record "Item Journal Line"; TransferLine: Record "Transfer Line"; TransferShipmentLine: Record "Transfer Shipment Line")
    var
    begin
        ItemJournalLine."Capex No." := TransferLine."Capex No.";
    end;
    ...
    var
    begin
        TransShptLine."Capex No." := TransLine."Capex No.";
    end;
    ...
    begin
        ...
        AssociateVendorMgt.GenerateItemLedgerEntries(NewPurchRcptLine);
