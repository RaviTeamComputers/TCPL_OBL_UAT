table 50084 "Budget Master"
{
    ...
    var
        ...
        InstreamVar, GInstream : InStream;
 ...
 */
            ...
            BudgetMaster1.SETFILTER("No.", '%1', BudgetMaster."No.");
            if BudgetMaster1.FindFirst() then begin
                ...
                ProjectBudgetReport.SaveAs('', ReportFormat::Pdf, OutstreamVar);
                TempBlobCU.CreateInStream(InstreamVar);

                // MESSAGE(DestinationFileName);
                // CopyStream(OutstreamVar, InstreamVar);
                EmailMsg.AddAttachment('Capex -' + BudgetMaster."Posting No." + '.pdf', 'application/pdf', Base64.ToBase64(InstreamVar));
            ...
            AttachmentManagment.RESET;
            AttachmentManagment.SETFILTER("Source Document No.", '%1', BudgetMaster."No.");
            IF AttachmentManagment.FINDFIRST THEN BEGIN
                REPEAT
                    ...
                    IF AttachmentManagment.Content.HASVALUE THEN BEGIN
                        ...
                        // TempBlobCU.CreateInStream(InstreamVar);
                        //ExportFileName := AttachmentManagment."File Name" + '.' + AttachmentManagment.Extension;
                        AttachmentManagment.CalcFields(Content);
                        if not AttachmentManagment.Content.HasValue then
                            exit;
                        AttachmentManagment.Content.CreateInStream(InstreamVar);
    ...
    procedure getexecutivesummarynew(budget_master: Record "Budget Master") executivesummary: Text
    var
        executiveInStream: InStream;
        TempBlob: Codeunit "Temp Blob";
        ConvertBase64: codeunit "Base64 Convert";
    begin
        executivesummary := '';
        budget_master.CalcFields("Executive Summary");

        if budget_master."Executive Summary".HasValue() then begin
            Clear(executiveInStream);
            TempBlob.CreateInStream(executiveInStream);

            budget_master."Executive Summary".CreateInStream(executiveInStream);
            //executiveInStream.Read(executivesummary);

            executivesummary := ConvertBase64.ToBase64(executiveInStream);
            if executivesummary <> '' then
                Message('inside getexecutivesummarynew : after calculation executive summary : %1', executivesummary);
